...
theme.getScrollParent = (node) => {
  ...
  const style = isElement ? window.getComputedStyle(node) : null;
  ...
  const parent = theme.getScrollParent(node.parentNode);

  // If no parent scroll container found, we are at the root
...
theme.scrollToRevealElement = (el) => {
  ...
  const onScrollEnd = debounce(() => {
    ...
    if (elTop >= top + theme.stickyHeaderHeight() && elTop <= top + scrollVisibleHeight - 50) {
...
theme.suffixIds = (container, prefix) => {
  const suffixCandidates = ['id', 'for', 'form', 'aria-describedby', 'aria-controls', 'data-target-id', 'data-modal'];
...
const MainNavigation = class extends HTMLElement {
  ...
  connectedCallback() {
    ...
    const headerSocial = document.querySelector('page-header .social');
    if (headerSocial) {
      const clone = headerSocial.cloneNode(true);
      clone.classList.remove('desktop-only');
      mobileDrawerFooter.appendChild(clone);
    }

    const headerExtraText = document.querySelector('page-header .icon_with_text');
    if (headerExtraText) {
      const clone = headerExtraText.cloneNode(true);
      // clone.classList.remove('desktop-only');
      mobileDrawerFooter.appendChild(clone);
    }
    ...
    theme.addDelegateEventListener(this.mobileDrawer, 'click', '.mobile-nav-back', (evt) => {
      ...
      this.mobileDrawer.querySelectorAll('.navigation__tier-1 > .navigation__item--open').forEach((el) => {
        ...
        let delay = 500;
        try {
          delay = parseFloat(getComputedStyle(link).transitionDuration.split(', ')[0]) * 1000;
        } catch (ex) {
          // do nothing
        }
        setTimeout(() => link.focus(), delay);
      });
    });
...
const MediaGallery = class extends HTMLElement {
  ...
  setActiveMedia(mediaId, scrollToItem = true, firstLoad = false) {
    ...
    if (scrollToItem) {
      ...
      if (this.collage
        && this.collage.offsetParent
        && !firstLoad
        && !this.hasAttribute('data-no-collage-variant-highlight')
      ) {
...
if (theme.settings.quickbuyStyle !== 'off') {
  theme.quickBuy = {
          ...
          if (block.classList.contains('expanded')) {
            ...
            fetch(productUrl, {
              ...
              .then((response) => {
                ...
                // include size guide sections
                tmpl.content.querySelectorAll('.section-size-guide').forEach((el) => {
                  // convert to modal, if not one already
                  if (el.querySelector('.modal--for-size-guide')) {
                    newDetail.insertAdjacentElement('beforeend', el.cloneNode(true));
                  } else {
                    const sizeGuide = el.querySelector('[data-modal-template]').content.querySelector('size-guide').cloneNode(true);
                    sizeGuide.querySelector('.modal__content').appendChild(el.querySelector('.size-guide__content'));
                    newDetail.insertAdjacentElement('beforeend', sizeGuide);
                  }
                });
                ...
                tmpl.content.querySelectorAll(':is(.section-main-product, .section-size-guide) link[rel="stylesheet"]').forEach((el) => {
                ...
                tmpl.content.querySelectorAll(':is(.section-main-product, .section-size-guide) script[src]').forEach((el) => {
    ...
    convertToQuickBuyContent: (container, productUrl) => {
      ...
      container.querySelectorAll('.sticky-column-desktop').forEach((el) => { el.classList.remove('sticky-column-desktop'); });
      ...
      theme.suffixIds(container, 'QuickBuy');
...
class CarouselSlider extends HTMLElement {
  ...
  scrollToElement(el, transition) {
    if (!this.hasAttribute('loaded')) this.refresh(); // Ensure loaded when called from outside
...
const ToggleTarget = class extends HTMLElement {
  constructor() {
    ...
    if (this.dataset.toggleTarget[0] === '#') {
      const target = document.querySelector(this.dataset.toggleTarget);
      if (target) {
        this.setAttribute('aria-controls', target.id);
        this.setAttribute('aria-expanded', !target.classList.contains('toggle-target--hidden'));
      }
    }
  }
...
const PageHeader = class extends HTMLElement {
  constructor() {
    ...
    if (mainSearch) {
      theme.addDelegateEventListener(this, 'click', '.show-search-link', (evt, currentTarget) => {
        evt.preventDefault();
        theme.manuallyLoadImages(mainSearch);
        document.body.classList.add('show-search');
        setTimeout(() => {
          theme.trapOpener = currentTarget;
          trapFocus(mainSearch, this.querySelector('.main-search__input'));
        }, 500);
      });
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mobile nav
  theme.addDelegateEventListener(document, 'click', '.mobile-nav-toggle', (evt, currentTarget) => {
    evt.preventDefault();
    if (document.body.classList.contains('enable-mobile-nav-transition')) {
      ...
      theme.openMobileNav();
      theme.trapOpener = currentTarget;
      trapFocus(document.querySelector('.mobile-navigation-drawer'), document.querySelector('.mobile-navigation-drawer .mobile-nav-toggle'));
    }
  });
